...
export default function SubAdminNotifications() {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const list = result.items || result.data?.items || (Array.isArray(result) ? result : []);
  ...
  return (
    <SubAdminLayout>
      <Box sx={{ pb: 3 }}>
        <PageHeader title="Notifications" crumbs={[{ label: "Notifications" }]} homePath="/sub-admin/dashboard" />

        {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}

        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Typography sx={{ color: "#64748B", fontSize: "0.9rem" }}>
            {loading ? "Loading..." : `${unread} unread · ${items.length - unread} read`}
          </Typography>
          {unread > 0 && (
        ...
        {loading ? (
          ...
          <Box sx={{ ...card, textAlign: "center", py: 4, color: "#64748B" }}>No notifications</Box>
        ) : items.map((n) => {
          const isRead = n.isRead || n.read;
          return (
            <Box key={n.id} sx={{ ...card, mb: 1.5, display: "flex", justifyContent: "space-between", borderLeft: isRead ? undefined : "3px solid #2563EB", opacity: isRead ? 0.75 : 1 }}>
              <Box sx={{ flex: 1, mr: 2 }}>
                <Box display="flex" alignItems="center" gap={1} mb={0.5} flexWrap="wrap">
                  ...
                  {!isRead && <Chip label="Unread" size="small" sx={{ height: 20, fontSize: "0.65rem", bgcolor: "#EFF6FF", color: "#2563EB" }} />}
                </Box>
                <Typography sx={{ color: "#64748B", fontSize: "0.85rem" }}>{n.message}</Typography>
                <Typography sx={{ color: "#94A3B8", fontSize: "0.75rem", mt: 0.5 }}>{formatTime(n.createdAt || n.time)}</Typography>
              </Box>
