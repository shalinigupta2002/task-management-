...
import { PageHeader, card } from "../../components/super-admin/shared";
import { getNotifications, setNotifications } from "../../utils/superAdminStorage";
  ...
  return (
    <SuperAdminLayout>
      <Box sx={{ pb: 3 }}>
        ...
        <Typography sx={{ color: "#64748B", mb: 2, fontSize: "0.9rem" }}>{unread} unread notifications</Typography>
        {items.map((n) => (
          <Box key={n.id} sx={{ ...card, mb: 1.5, display: "flex", justifyContent: "space-between", alignItems: "flex-start", opacity: n.read ? 0.75 : 1, borderLeft: n.read ? undefined : "3px solid #2563EB" }}>
            <Box sx={{ flex: 1, mr: 2 }}>
              <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                ...
                {!n.read && <Chip label="New" size="small" sx={{ height: 20, fontSize: "0.65rem", bgcolor: "#EFF6FF", color: "#2563EB" }} />}
              </Box>
