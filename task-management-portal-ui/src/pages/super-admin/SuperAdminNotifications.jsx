    ...
    <SuperAdminLayout>
      <Box sx={{ pb: 3 }}>
        ...
        <Typography sx={{ color: "#64748B", mb: 2, fontSize: "0.9rem" }}>
          {loading ? "Loading..." : `${unread} unread notifications`}
        </Typography>
        ...
        {loading ? (
          ...
          <Box sx={{ ...card, textAlign: "center", py: 6 }}>
            ...
            <Typography sx={{ color: "#64748B", fontSize: "0.9rem" }}>
              {error ? "Please try again later." : "Platform alerts will appear here when events occur."}
            </Typography>
          </Box>
        ) : (
          items.map((n) => {
              ...
              >
                <Box sx={{ flex: 1, mr: 2 }}>
                  <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                    ...
                    {!isRead && (
                      <Chip label="New" size="small" sx={{ height: 20, fontSize: "0.65rem", bgcolor: "#EFF6FF", color: "#2563EB" }} />
                    )}
                  </Box>
