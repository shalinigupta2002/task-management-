...
/** One-time legacy display code — used only to backfill existing rows without a stored code. */
export function legacyDisplayCodeFromName(name) {
  if (!name) return "GEN";
  const cleaned = name.replace(/[^a-zA-Z0-9]/g, "").substring(0, 4).toUpperCase();
  return cleaned || "GEN";
}
